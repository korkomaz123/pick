...
class OrderChangeNotifier extends ChangeNotifier {
  ...
  Future sendAsGift(String receiver, String message) async {
    var _res = await orderRepository.sendAsGift(user?.token, user.email, receiver, message);
    if (_res['code'] == 'SUCCESS') {
      return _res['gift_message_id'].toString();
    }
    return false;
  }
